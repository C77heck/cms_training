...
/* we require the third party files to be used. like the class below */
require 'includes/init.php';

$email = '';
$subject = '';
$message = '';
$sent = false; //set to false so the form is displayed
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        /* notice how we validate email input */
    ...
    if (empty($errors)) {
        ...
        /* passing true as argument will allow exceptions being thrown */

        try {
            /* this below is pretty standard like i did it in express. */
            $mail->isSMTP();
            $mail->Host = SMTP_HOST;
            $mail->SMTPAuth = true;
